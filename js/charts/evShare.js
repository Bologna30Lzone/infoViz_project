...
export function chartEVShare(rootEl, d3) {
  ...
  const btnTotal   = controls.append('button').text('Total View');
  const btnFocused = controls.append('button').text('Focused View');

  btnTotal.on('click', () => { currentMode = 'total'; render(); setActive(); });
  btnFocused.on('click', () => { currentMode = 'focused'; render(); setActive(); });
  ...
  function setActive() {
    ...
    (currentMode === 'total' ? btnTotal : btnFocused).classed('active', true)
  ...
  d3.csv(url).then(rows => {
    ...
    wrap.append('div').style('color', '#b91c1c').text('Unable to load EV data.');
  });
  ...
  function render(){
    ...
    const h = Math.max(140, height) - margin.top - margin.bottom - controls.node().offsetHeight;
    ...
    // Axes
    const xAxis = d3.axisBottom(x).ticks(Math.min(6, data.length)).tickFormat(d3.timeFormat('%Y'));
    ...
    g.append('text')
      ...
      .text(currentMode === 'total' ? 'Electric vs Non-Electric Cars' : 'Electric Cars Growth (Focused)');

    if (currentMode === 'total') {
      // Areas (mirroring your original script.js logic)
      const areaElectric = d3.area()
        .x(d => x(d.year))
        .y0(h)
        .y1(d => y(d.percentage));

      const areaNonElectric = d3.area()
        .x(d => x(d.year))
        .y0(d => y(d.percentage))
        .y1(0);
      ...
      const legendX = w - 150, legendY = Math.max(24, h * 0.55);
      ...
      legend.append('rect').attr('x', 0).attr('y', 0).attr('width', 15).attr('height', 15)
        .attr('fill', '#ffffff').attr('stroke', '#222').attr('stroke-width', 1);
      legend.append('text').attr('x', 20).attr('y', 12).style('font-size', '12px')
        .text('Non-Electric Cars (%)');

      // Electric
      legend.append('rect').attr('x', 0).attr('y', 20).attr('width', 15).attr('height', 15)
        .attr('fill', '#C81515').attr('stroke', '#222').attr('stroke-width', 1);
      const lab = legend.append('text').attr('x', 20).attr('y', 32).style('font-size', '12px')
        .text('Electric Cars (%)');
      ...
      try {
        ...
        for (let i = 0; i < data.length - 1; i++) {
          if (data[i].year <= dateAtLegendX && data[i+1].year >= dateAtLegendX) { lo = data[i]; hi = data[i+1]; break; }
        }
        ...
        if (yStart <= yAt) {
          g.append('line')
            .attr('x1', legendCenterX).attr('y1', yStart)
            .attr('x2', legendCenterX).attr('y2', yAt)
      ...
      // Simple legend
      const legend = g.append('g').attr('transform', `translate(${w - 150}, 8)`);
      legend.append('rect').attr('width', 15).attr('height', 15).attr('fill', '#C81515');
      legend.append('text').attr('x', 20).attr('y', 12).style('font-size','12px').text('Electric Cars (%)');
    }

    // Top/right border (as in your original)
    g.append('line').attr('x1', 0).attr('x2', w).attr('y1', 0).attr('y2', 0)
      .attr('stroke', '#000').attr('stroke-width', 1).attr('shape-rendering','crispEdges');
    g.append('line').attr('x1', w).attr('x2', w).attr('y1', 0).attr('y2', h)
      .attr('stroke', '#000').attr('stroke-width', 1).attr('shape-rendering','crispEdges');
  }
